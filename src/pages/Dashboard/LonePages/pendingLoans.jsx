...
const PendingLoans = () => {
  ...
  const openModal = (loan) => { setSelectedLoan(loan); setIsModalOpen(true); };
  const closeModal = () => { setSelectedLoan(null); setIsModalOpen(false); };
            ...
            {loans.length > 0 ? loans.map((loan, index) => (
              <tr key={loan._id}>
                ...
                <td className="flex flex-col sm:flex-row gap-2">
                  <button onClick={() => handleApprove(loan._id)} className="btn btn-sm btn-success flex-1">Approve</button>
                  <button onClick={() => handleReject(loan._id)} className="btn btn-sm btn-error flex-1">Reject</button>
                  <button onClick={() => openModal(loan)} className="btn btn-sm btn-info flex-1">View</button>
                </td>
              </tr>
            )) : (
              <tr><td colSpan="6" className="text-center py-6 text-gray-500">No pending loans found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {isModalOpen && selectedLoan && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-base-100 rounded-2xl p-6 w-full max-w-2xl relative shadow-lg">
            <button className="absolute top-4 right-4 btn btn-sm btn-circle btn-ghost" onClick={closeModal}>✕</button>
            ...
            <div className="space-y-3">
              ...
              <p><span className="font-semibold">Borrower:</span> {selectedLoan.firstName} {selectedLoan.lastName} ({selectedLoan.userEmail})</p>
              <p><span className="font-semibold">Amount:</span> {selectedLoan.loanAmount}</p>
              <p><span className="font-semibold">Date:</span> {selectedLoan.date}</p>
              <p><span className="font-semibold">Status:</span> {selectedLoan.status}</p>
              <p><span className="font-semibold">Reason:</span> {selectedLoan.loanReason || "N/A"}</p>
              <p><span className="font-semibold">Extra Notes:</span> {selectedLoan.extraNotes || "N/A"}</p>
            </div>
          </div>
        </div>
      )}
